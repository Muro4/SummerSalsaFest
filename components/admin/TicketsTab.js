...
export default function TicketsTab({ tickets, users, onStageChange, historyStagedData }) {
   ...
   const filteredTickets = tickets.filter(t => {
      ...
      const matchesSearch = t.userName?.toLowerCase().includes(searchTerm.toLowerCase()) || t.ticketID?.toLowerCase().includes(searchTerm.toLowerCase()) || ambTag.toLowerCase().includes(searchTerm.toLowerCase());
   ...
   return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
         ...
         <div className="lg:hidden flex flex-col gap-4 relative z-10 pb-20">
            ...
            {filteredTickets.length === 0 && <div className="bg-white rounded-3xl p-10 text-center text-slate-400 text-xs font-bold uppercase tracking-widest border border-gray-100">No tickets match your search.</div>}
         </div>
      </div>
   );
}