      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
         ...
         <div className="bg-white rounded-[3rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col relative z-10">
            ...
            <div className="p-8 md:p-10 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-50/50 shrink-0 gap-6 rounded-t-[3rem]">
               <div>
                  <h2 className="font-bebas tracking-wide text-4xl text-slate-900 uppercase tracking-wide">{t('title')}</h2>
                  <p className="text-xs font-medium text-slate-500 mt-1 font-montserrat">{t('subtitle')}</p>
               </div>
               <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                  <div className="flex items-center gap-2">
                     <span className="text-[11px] font-bold uppercase text-slate-400 tracking-widest font-montserrat">{t('registered')}</span>
                     <span className="font-bebas text-4xl leading-none text-slate-900">{filteredHistory.length}</span>
                  </div>
                  <div className="flex items-center gap-2">
                     <span className="text-[11px] font-bold uppercase text-slate-400 tracking-widest font-montserrat">{t('totalPaid')}</span>
            ...
            <div className="hidden lg:block overflow-x-auto w-full flex-grow pb-40">
               <table className="w-full text-left border-separate border-spacing-0 min-w-[950px] font-montserrat relative">
                  <thead className="bg-white text-[11px] font-bold uppercase text-slate-400 tracking-widest relative z-10">
                     <tr>
                        <th className="p-6 pl-10 font-bold w-1/3 border-b border-gray-100">{t('thName')}</th>
                        <th className="p-6 font-bold text-center w-48 border-b border-gray-100">{t('thPassType')}</th>
                        <th className="p-6 font-bold text-right w-32 border-b border-gray-100">{t('thPrice')}</th>
                        <th className="p-6 pl-16 font-bold text-center w-56 border-b border-gray-100">{t('thEmailed')}</th>
                        <th className="p-6 pr-10 text-right font-bold w-40 border-b border-gray-100">{t('thStatus')}</th>
                     </tr>
                  </thead>
                  <tbody className="uppercase text-xs font-bold text-slate-900">
                     {filteredHistory.map((ticket, i) => (
                        <tr key={ticket.id} onClick={() => setFullScreenTicket(ticket)} className="hover:bg-slate-50/50 transition-colors cursor-pointer group">
                           <td className="p-6 pl-10 align-middle border-b border-gray-50 max-w-[300px] xl:max-w-[400px]">
                              <div className="flex items-center gap-4 h-full mt-1">
                                 ...
                                 <div className="min-w-0 flex-1">
                                    <span title={ticket.userName} className="block text-base font-bold font-montserrat text-slate-700 tracking-wide group-hover:text-salsa-pink transition-colors leading-tight truncate">{ticket.userName}</span>
                                 </div>
                              </div>
                           </td>
                           ...
                           <td className="p-6 text-base font-montserrat font-bold text-slate-700 tracking-wide text-right align-middle border-b border-gray-50">€{ticket.price}</td>
                           <td className="p-6 pl-16 text-center align-middle font-montserrat border-b border-gray-50">
                              <div className="flex items-center justify-center h-full mt-1">
                                 {ticket.emailSentCount > 0 ? (
                                    <span className="inline-flex items-center gap-1.5 text-[10px] font-black text-emerald-500 bg-emerald-50 px-3 py-1.5 rounded-md tracking-widest"><Mail size={12} /> {ticket.emailSentCount}</span>
                                 ) : (
                           ...
                           <td className="p-6 pr-10 align-middle font-montserrat border-b border-gray-50">
                              <div className="flex items-center justify-end gap-4 h-full mt-1">
                                 <div className={`flex items-center gap-1.5 text-[11px] font-black tracking-widest uppercase ${ticket.status === 'active' ? 'text-emerald-500' : 'text-amber-500'}`}>
                                    {ticket.status === 'active' ? <CheckCircle size={14} /> : <Clock size={14} />} {ticket.status === 'active' ? t('statusActive') : t('statusPending')}
                                 </div>
                                 <div className="bg-white p-2 rounded-xl text-slate-400 group-hover:bg-salsa-pink group-hover:text-white transition-all duration-300 shadow-sm border border-gray-200 group-hover:border-salsa-pink group-hover:scale-110" title={t('viewTicket')}>
                                    <Eye size={16} />
                                 </div>
                              </div>
                           </td>
                     ...
                     {filteredHistory.length === 0 && <tr><td colSpan="5" className="p-12 text-center text-slate-400 text-xs font-bold uppercase tracking-widest font-montserrat border-b border-gray-50">{t('noPaidFound')}</td></tr>}
                  </tbody>
               </table>
            </div>
            ...
            <div className="lg:hidden flex flex-col gap-4 p-4 bg-slate-50 border-t border-gray-100 flex-grow pb-24">
               {filteredHistory.map((ticket) => (
                  <div key={ticket.id} onClick={() => setFullScreenTicket(ticket)} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm flex flex-col gap-3 relative cursor-pointer hover:ring-2 ring-salsa-pink/50 transition-all">
                     
                     {/* Header Info */}
                     <div className="flex justify-between items-start gap-4">
                        <div className="flex-1 min-w-0 pr-2">
                           <span title={ticket.userName} className="block text-lg font-black font-montserrat text-slate-900 uppercase leading-tight tracking-widest truncate">{ticket.userName}</span>
                           <span className="block text-sm font-bold text-slate-500 mt-1.5 uppercase tracking-widest font-mono truncate">{t('lblId')}: {ticket.ticketID}</span>
                        </div>
                        {/* Pass Pill (Fixed Width) */}
                        <span className={`shrink-0 inline-flex items-center justify-center w-24 py-1.5 rounded-full text-[10px] shadow-sm font-black uppercase tracking-widest mt-1 ${getPassStyle(ticket.passType)}`}>
                           {translatePassDisplay(ticket.passType)}
                        </span>
                     </div>
                     ...
                     <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-1 w-full">
                        <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest">
                           {ticket.status === 'active' ? <CheckCircle size={14} className="text-emerald-500"/> : <Clock size={14} className="text-amber-500"/>}
                           <span className={ticket.status === 'active' ? 'text-emerald-500' : 'text-amber-500'}>
   {ticket.status === 'active' ? t('statusActive') : t('statusPending')}
</span>
                        </div>
                        
                        <div className="flex items-center gap-3">
                           {ticket.emailSentCount > 0 && <span className="text-emerald-500 bg-emerald-50 px-2 py-1 rounded-md text-[10px] font-bold tracking-widest flex items-center gap-1"><Mail size={12}/> {ticket.emailSentCount}</span>}
                           <span className="font-bold text-slate-700 text-sm">€{ticket.price}</span>
                        </div>
                     </div>
