...
export default function Home() {
  ...
  // Track if the user is currently "manually" controlling it via arrows
  ...
  const stopAutoScrollAndScheduleRestart = () => {
    // 1. Kill the auto-scroll immediately
    if (autoScrollInterval.current) clearInterval(autoScrollInterval.current);
    autoScrollInterval.current = null;
    setIsManuallyScrolling(true);

    // 2. Clear any existing restart timers
    if (restartTimeoutRef.current) clearTimeout(restartTimeoutRef.current);

    // 3. Set a 5 second timer to restart the animation
    ...
    <main className="min-h-screen bg-white font-montserrat overflow-x-hidden">
      ...
      <section id="info" className="py-32 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-20">
          ...
          <h2 className="font-modak text-6xl text-gray-900 mt-2">Why Summer Salsa?</h2>
      ...
      <section className="py-32 bg-gray-50 relative overflow-hidden">
        ...
        <div className="relative max-w-[100vw] mx-auto group">
          <button
            onClick={scrollLeft}
            className="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 z-20 p-4 bg-white/90 backdrop-blur-sm border border-gray-200 text-gray-900 rounded-full hover:border-salsa-pink hover:text-salsa-pink transition-all shadow-xl opacity-0 group-hover:opacity-100 hidden md:block"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <button
            onClick={scrollRight}
            className="absolute right-4 md:right-12 top-1/2 -translate-y-1/2 z-20 p-4 bg-white/90 backdrop-blur-sm border border-gray-200 text-gray-900 rounded-full hover:border-salsa-pink hover:text-salsa-pink transition-all shadow-xl opacity-0 group-hover:opacity-100 hidden md:block"
          >
          ...
          <div
            ...
            // Removed Hover Listeners to ensure it ONLY stops on Arrow Click
            className="flex gap-8 px-6 md:px-[15vw] overflow-x-auto custom-scrollbar py-10 pb-14 scroll-smooth"
          >
            {[...reviews, ...reviews, ...reviews].map((review, i) => (
              <div
                key={i}
                className="shrink-0 w-[300px] md:w-[400px] bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/50 transition-transform hover:-translate-y-2 flex flex-col"
              >
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, s) => <Star key={s} size={16} className="fill-salsa-pink text-salsa-pink" />)}
                </div>

                <p className="italic text-gray-600 text-lg mb-8 leading-relaxed font-medium flex-grow">
                  "{review.text}"
                </p>
          ...
          <div className="absolute top-0 left-0 w-[10vw] h-full bg-gradient-to-r from-gray-50 to-transparent z-10 pointer-events-none hidden md:block"></div>
          <div className="absolute top-0 right-0 w-[10vw] h-full bg-gradient-to-l from-gray-50 to-transparent z-10 pointer-events-none hidden md:block"></div>
        </div>
      </section>
