...
export default function Home() {
  ...
  // The Cards Data (We pass the icon reference itself, not the JSX, so we can reuse it)
  const features = [
  ...
  useEffect(() => {
    // 1. Calculate the Dynamic Year safely on the client
    const d = new Date();
    // If we are past August (month index 7), show NEXT year's festival
    const calculatedYear = d.getMonth() > 7 ? d.getFullYear() + 1 : d.getFullYear();
    setFestivalYear(calculatedYear);
    
    // 2. Calculate the Edition (Assuming 2026 is the 15th Edition -> Base Year is 2011)
    const editionNumber = calculatedYear - 2011;
    
    // Quick helper to add "st", "nd", "rd", "th"
    const getOrdinal = (n) => {
      const s = ["th", "st", "nd", "rd"];
      const v = n % 100;
    ...
    <main className="min-h-screen bg-white font-montserrat overflow-x-hidden">
      ...
      >
        ...
        <div className="relative z-10 w-full h-screen flex flex-col items-center justify-center">
          <div className="text-salsa-white max-w-6xl mt-12 flex flex-col items-center">
            ...
            <div className="animate-fade-in delay-700 flex flex-col sm:flex-row items-center justify-center gap-6">
              ...
              <Button href="/info" variant="ghost" size="lg" className="w-72 border-2 border-white/40 text-white hover:bg-white hover:text-slate-900 backdrop-blur-sm">
                LEARN MORE
              </Button>
            </div>
          </div>
        ...
        <div className="absolute -bottom-[1px] left-0 w-full z-20 leading-[0]">
          <svg className="w-full h-16 md:h-24 block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
      ...
      <section id="info" className="py-32 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {features.map((item, i) => {
            ...
            return (
              <div key={i} className="relative p-10 bg-gradient-to-br from-white to-slate-50 border border-slate-100 rounded-[2.5rem] shadow-xl shadow-slate-200/40 overflow-hidden">
                
                {/* Decorative Faded Background Icon */}
                <Icon className="absolute -bottom-6 -right-6 w-48 h-48 text-slate-200 opacity-30 pointer-events-none" />
                
                <div className="relative z-10">
                  {/* Styled Icon Container */}
                  <div className="mb-8 w-16 h-16 bg-salsa-pink/10 rounded-2xl flex items-center justify-center">
                    <Icon className="text-salsa-pink w-8 h-8" />
                  </div>
      ...
      <section className="py-32 bg-slate-50 relative overflow-hidden">
        ...
        <div className="relative max-w-[100vw] mx-auto group">
          <div className="absolute left-4 md:left-12 top-1/2 -translate-y-1/2 z-20 hidden md:block opacity-0 group-hover:opacity-100 transition-opacity">
            <Button variant="actionIcon" size="icon" icon={ChevronLeft} onClick={scrollLeft} className="p-4 bg-white/90 backdrop-blur-sm border border-slate-200 text-slate-900 rounded-full hover:border-salsa-pink hover:text-salsa-pink shadow-xl"/>
          </div>
      ...
      <section className="py-32 px-6 text-center bg-salsa-white text-slate-900">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-6">
          ...
          <Button href="/contact" variant="outline" size="lg" className="w-72 border-slate-900 text-slate-900 hover:bg-slate-900 hover:text-white">
            CONTACT US
          </Button>
        </div>
      </section>
