...
import InboxManager from "@/components/admin/InboxManager"; // NEW INBOX IMPORT
import {
   Users, Ticket, Search, Download,
   ShieldAlert, Gift, Filter, CheckCircle,
   Clock, Trash2, Loader2, BarChart3, UserCog,
   Undo2, Redo2, Save, Eye, Mail // Added Mail icon here
} from "lucide-react";
      ...
      <main className="min-h-screen bg-salsa-white font-montserrat pt-32 pb-24 select-none">
         ...
         <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12 gap-6 w-full">
               ...
               {/* 4-WAY ANIMATED TABS (UPDATED WIDTH & MATH) */}
               <div className="relative flex bg-slate-50 border border-gray-100 p-1.5 rounded-2xl w-full lg:w-[550px] shadow-[inset_0_2px_8px_rgba(0,0,0,0.04)]">
                  <div
                     className="absolute top-1.5 bottom-1.5 w-[calc((100%-0.75rem)/4)] bg-slate-900 rounded-xl transition-all duration-300 ease-out shadow-sm"
                     style={{
                  ...
                  />
                  <button onClick={() => setActiveTab("analytics")} className={`tab-button z-10 cursor-pointer ${activeTab === 'analytics' ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}><BarChart3 size={14} /> Analytics</button>
                  <button onClick={() => setActiveTab("inbox")} className={`tab-button z-10 cursor-pointer ${activeTab === 'inbox' ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}><Mail size={14} /> Inbox</button>
                  <button onClick={() => setActiveTab("tickets")} className={`tab-button z-10 cursor-pointer ${activeTab === 'tickets' ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}><Ticket size={14} /> Tickets</button>
                  <button onClick={() => setActiveTab("users")} className={`tab-button z-10 cursor-pointer ${activeTab === 'users' ? 'text-white' : 'text-slate-400 hover:text-slate-600'}`}><UserCog size={14} /> Users</button>
               </div>
            ...
            <div className="relative min-h-[500px] w-full">
               ...
               {/* INBOX TAB */}
               {activeTab === 'inbox' && (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 z-20 relative">
                     <InboxManager />
                  </div>
               )}
