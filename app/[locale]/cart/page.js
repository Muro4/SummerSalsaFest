  ...
  const handleCheckout = async () => {
    ...
    if (total === 0) {
      try {
        ...
        showPopup({ type: "error", title: t('popupActErrTitle'), message: t('popupActErrMsg'), confirmText: t('popupCloseBtn') });
        setIsPaying(false);
        return;
      }
    }
      ...
      <div className="flex-grow max-w-7xl mx-auto px-4 md:px-6 w-full pt-32 md:pt-40 pb-24">
        ...
        {items.length > 0 ? (
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-16">
            ...
            <div className="lg:col-span-5 xl:col-span-4 mt-4 lg:mt-0">
              <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-2xl sticky top-32 w-full">
                <h2 className="font-bebas tracking-wide text-3xl md:text-4xl mb-5 md:mb-6 uppercase text-slate-900 tracking-wide">{t('summaryTitle')}</h2>
                  ...
                  {Object.entries(counts).map(([type, count], index) => (
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        {/* TRANSLATED PASS TYPE IN SUMMARY */}
                ...
                >
                  {isPaying ? (
                    ...
                    total === 0 ? <><CheckCircle size={16} /> {t('activateBtn')}</> : <><Lock size={16} /> {t('proceedBtn')}</>
                  )}
                </button>
                ...
                <div className="mt-5 flex items-center justify-center gap-2 text-[9px] md:text-[11px] font-black text-slate-300 uppercase tracking-widest">
                  {total > 0 ? (
                    <><ShieldCheck size={12} className="text-emerald-400" /> {t('secureChk')}</>
                  ) : (
                    <><ShieldCheck size={12} className="text-emerald-400" /> {t('freeProc')}</>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="w-full border-2 border-dashed border-salsa-mint/40 bg-[#f4fdfb] rounded-[2rem] md:rounded-[3rem] py-20 md:py-32 flex flex-col items-center justify-center text-center animate-in fade-in duration-500">
            ...
            <h3 className="font-bebas tracking-wide text-3xl md:text-4xl text-slate-400 tracking-wide uppercase mb-3 md:mb-4">{t('emptyTitle')}</h3>
