...
export default function Login() {
  ...
  return (
    <main className="min-h-screen bg-salsa-white flex items-center justify-center p-4 font-montserrat relative">
      ...
      {/* GLOBAL BACK BUTTON */}
      <Link href="/" className="absolute top-8 left-8 flex items-center gap-2 text-slate-500 hover:text-slate-900 font-black text-[10px] uppercase tracking-widest transition-colors z-50">
      ...
      <div className="relative w-full max-w-4xl min-h-[700px] bg-white rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden flex flex-col md:flex-row mt-12 md:mt-0">
        ...
        {/* =========================================
            MOBILE VIEW (Fades between forms)
        ========================================= */}
        <div className="md:hidden flex flex-col w-full p-8 relative z-10">
          ...
          <button onClick={handleGoogleAuth} className="cursor-pointer w-full flex items-center justify-center gap-3 border-2 border-gray-100 py-4 rounded-2xl hover:bg-gray-50 transition mb-6 font-black tracking-widest text-xs uppercase text-slate-700">
        ...
        {/* =========================================
            DESKTOP VIEW (The Sliding Panel UI)
        ========================================= */}
        ...
        <div className="hidden md:flex flex-col justify-center w-1/2 p-12 absolute left-0 h-full z-10">
          ...
          <button onClick={handleGoogleAuth} className="cursor-pointer w-full flex items-center justify-center gap-3 border-2 border-gray-100 py-3 rounded-2xl hover:bg-salsa-pink hover:text-white hover:border-salsa-pink transition-all mb-6 font-black tracking-widest text-[10px] uppercase text-gray-500 mt-4 group">
          ...
          <div className="relative mb-6 text-center">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-gray-100"></div></div>
            <div className="relative inline-block bg-white px-4 text-[10px] uppercase font-black text-gray-400 tracking-widest">Or use email</div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1">
              ...
              <div className="relative flex items-center">
                <input required type="email" value={email} onChange={e => setEmail(e.target.value)} className={`w-full bg-gray-50 border text-slate-900 font-medium rounded-2xl px-6 py-4 pl-12 focus:bg-white focus:ring-2 transition-all outline-none ${isEmailInvalid ? 'border-red-400 focus:border-red-500 focus:ring-red-500/30' : 'border-gray-200 focus:border-salsa-mint focus:ring-salsa-mint/30'}`} />
                <Mail className={`absolute left-4 ${isEmailInvalid ? 'text-red-400' : 'text-gray-400'}`} size={18} />
              </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative flex items-center">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="cursor-pointer absolute right-4 text-gray-400 hover:text-slate-900 transition">
        ...
        <div className="hidden md:flex flex-col justify-center w-1/2 p-12 absolute right-0 h-full z-10">
          ...
          <form onSubmit={handleSignUp} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2">Create Password</label>
              <div className="relative flex items-center">
                ...
                <Lock className={`absolute left-4 ${isPasswordInvalid ? 'text-red-400' : 'text-gray-400'}`} size={18} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="cursor-pointer absolute right-4 text-gray-400 hover:text-slate-900 transition">
                  {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
              </div>
              ...
              {/* DYNAMIC PASSWORD REQUIREMENTS DROPDOWN */}
              <div className={`mt-2 bg-gray-50 rounded-xl overflow-hidden border ${isPasswordInvalid ? 'border-red-200 bg-red-50' : 'border-gray-100'}`}>
                <button type="button" onClick={() => setShowReqs(!showReqs)} className={`cursor-pointer w-full flex items-center justify-between p-3 text-[10px] font-black uppercase tracking-widest transition-colors ${isPasswordInvalid ? 'text-red-500' : 'text-gray-500 hover:text-slate-900'}`}>
        ...
        {/* =========================================
            THE MAGIC SLIDING OVERLAY (More Saturated Mint)
        ========================================= */}
        <div className={`hidden md:flex absolute top-0 left-0 w-1/2 h-full z-30 transition-transform duration-700 ease-in-out ${isLogin ? 'translate-x-full' : 'translate-x-0'}`}>
          ...
          <div className="relative w-full h-full flex flex-col items-center justify-center text-center p-12">
            ...
            <div className={`absolute inset-0 flex flex-col items-center justify-center px-12 transition-all duration-700 ${isLogin ? 'opacity-100 translate-x-0 delay-300 pointer-events-auto' : 'opacity-0 -translate-x-10 pointer-events-none'}`}>
              ...
              <p className="font-bold text-slate-700 leading-relaxed mb-8">
              ...
              {/* Button hovers dark mint (teal-600) */}
